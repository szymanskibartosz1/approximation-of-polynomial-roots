function wynik = przybliz(a, x_pocz)
    % Funkcja wyznaczająca zera wielomianu o współczynnikach a
    % reprezentowanego w bazie wielomianów Czebyszewa I-go rodzaju, z
    % przybliżeniem początkowym w x_pocz

    % Sprawdzamy poprawność wprowadzonych danych
    if ~(isreal(a) && isreal(x_pocz))
        error('Argumenty muszą być rzeczywiste!')
    elseif isempty(a) || isempty(x_pocz)
        error('Podane argumenty są puste!')
    elseif length(a) == 1
        error('Nie można zastosować metody dla a długości 1!')
    elseif length(a) == 2 && a(2) == 0
        error('Nie można zastosować metody!')
    end


    i = 0; %Indeks do liczenia pętli
    x_k = x_pocz;
    x_pop = x_pocz+1; % Zeby petla zaczela dzialac przyjmujemy np. tyle
    d1 = 10^(-25); % Wartosci do warunku Gilla
    d2 = 10^(-35); % Wartosci do warunku Gilla
    while abs(x_pop-x_k) > d1*abs(x_k) + d2 % Warunek Gilla
        [wartosc_funkcji, wartosc_pochodnej, wartosc_drugiej_pochodnej] = wartosci(a, x_k);
        x_pop = x_k; % Zapisujemy poprzednią wartość
        x_k = x_k - ((wartosc_funkcji)/(wartosc_pochodnej - (wartosc_drugiej_pochodnej*wartosc_funkcji/(2*wartosc_pochodnej)))); % Obliczamy x_k+1 z metody Halley'a
        i = i +1;
        if i == 10000000 % Warunek 'bezpieczeństwa', żeby pętla nie wykonywała się zbyt wiele razy
            break;
        end
    end
    wynik = x_k; % Zwracamy wynik
